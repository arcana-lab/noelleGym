...
def readData(pathToFile):
  ...
  with open(pathToFile, 'r') as f:
    for line in f:
      ...
      llvm = float(lineAsList[1])
      noelle = float(lineAsList[2])
  ...
  for bsuite in bsuites:
    for elem in data[bsuite]:
      ...
      if (total == 0.0):
        ...
        llvm = float(elem[1])/total
      dataAll["LLVM"].append(llvm*100)
      dataAll["NOELLE"].append((noelle - llvm)*100) # it's relative

  return dataAll, bsuites, benchmarks, benchmarksPerBsuite

def plot(data, bsuites, benchmarks, benchmarksPerBsuite):
  compilers = ["LLVM", "NOELLE"]
  colorsMap = {"LLVM" : "purple", "NOELLE" : "orange"}
  ...
  # Set y label
  ax.set_ylabel('Induction Var [%]', fontsize = fontSize)
  ...
  # Save figure
  plt.savefig('./barplotInductionVariables.pdf', format = 'pdf')
